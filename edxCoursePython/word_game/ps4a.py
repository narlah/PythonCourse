...
def getWordScore(word="", n=0):
    ...
    returns: int >= 0
    """
    wordLowerised = word.lower()
    result = 0
    for letter in wordLowerised:
        result += SCRABBLE_LETTER_VALUES[letter]
    bonus = 0
    if word.__len__() >= n:
        bonus = 50
    return result * word.__len__() + bonus
...
def isValidWord(word="", hand={}, wordList=[]):
    ...
    for letter in word:
        if not local_hand.has_key(letter):
            return False
        else:
            local_hand[letter] -= 1

    for key in local_hand.keys():
        if local_hand[key] < 0:
            return False
    return True
...
def playHand(hand={}, wordList=[], n=0):
    """
...
def playGame(wordList):
    ...
    1) Asks the user to input 'n' or 'r' or 'e'.
      ...
      * If the user inputs 'r', lc1 et the user play the last hand again.
